...
// Block에 BlockHeader를 상속받고(부모 속성을 가져오고)
// IBlock 인터페이스 형태 클래스 만든다
export class Block extends BlockHeader implements IBlock {
  ...
  public data: string[];
  // constructor : class 객체의 초기값을 설정해줄 수 있다 
  ...
  public static isValidNewBlock(_newBlock: Block, _previousBlock: Block): Failable<Block, String> {
    ...
    if (_previousBlock.height + 1 !== _newBlock.height) return { isError: true, value: "블록 높이 오류" };
    // 블록의 이전 블록 해시 값이 새로운 블록의 이전 블록 해시값과 같은지
    if (_previousBlock.hash !== _newBlock.previousHash) return { isError: true, value: "이전 해시 오류" };
    // 생성된 블록의 정보를 가지고 다시 해싱해서 생성된 블록의 해시값과 같은지 비교
    if (Block.createBlockHash(_newBlock) !== _newBlock.hash) return { isError: true, value: "블록 해시 오류" };
