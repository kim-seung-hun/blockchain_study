...
// ---------------------------------------------------------------------------------------------
// 체인 만들기
// block 클래스로 만든 블록들을 체인으로 연결시켜줄 chain 클래스를 만들어보자
// chain 클래스에는 생성한 블록을 배열로 담아서 블록체인을 만들 예정
// 이미 지금까지 우리가 만든 블록은 블록의 속성으로 체이닝이 이루어지고 있는데
// 이전 블록 해시 값을 속성으로 가지고 있기 때문에 특정 블록 기준으로 이전 블록 해시 값이 달라지면
// 현재 블록의 이전 해시값과 불일치가 발생해서 연결 고리가 끊어진다
// chain 클래스를 따로 만들어서 생성된 블록을 하나의 배열 안에 담아주는 역할을 할 예정

// 이렇게 하는 이유는 이후에 우리가 마이닝 할 때 난이도 계산을 하기 위해서이다

// POW(Proof Of Work : 작업증명)

// 작업 증명의 역사
// 1993년 모니 나노어이 작업 증명의 기본 개념을 고안했다
// 1997년 영국의 암호학자인 애덤백의 해시캐시고 이후에
// 2009년 사토시 나카모토라는 사람이 비트코인에 적용해 오늘날까지 사용된다
...
// 논스값 사용
// 작업증명은 어려운 수학문제를 푸는 것
// 작업 증명을 하는 것이 채굴이라고 할 수 있다
// 어려운 수학 문제를 풀게해서 이 문제를 푼 사람한데 보상으로 코인을 주는 것
...
// 난이도가 4
// 논스??
// 0이 4개 이상인 해시값이 나올때까지 목표값이 나올때까지
// 논스를 0에서부터 계속 하나씩 더하며 해싱을 해서 목표값을 찾는 것
// 0이 4개이거나 혹은 6개여도 상관없도 목표값 이하의 값이 나오면 된다

// "작업 증명 방식"은 POW는 목표값 이하의 해시값을 찾는 과정을 무수히 반복해서
// 해당작업에 참여했음을 증명하는 방식의 알고리즘

// 작업 증명 알고리즘의 필요성은 네트워크 상의 모든 노드가 동시에 블록을 만들 수 없게 하는 것
// 새로운 블록을 추가 생성 할 수 있게 된다는 점

// 작업 증명 알고리즘은 Difficulty 조절 알고리즘을 이용해서
// 약 10분당 1개의 블록이 생성되는 것을 보장하게 된다
...
// 블록 한 개가 생성되는 예상 시간을 10분으로 설정하고
// 10개의 블록을 한 묶음으로 해서 블록 한 묶음이 생성되는 예상시간을
// 6000초라는 값을 할당해주고 이후 10개의 블록이 생성되는데 걸리는 시간
// timeExpected /2 보다 작을 경우 난이도 조절을 1 증가시키고
// timeExpected*2 보다 클 경우에는 난이도를 1감소시킨다

// P2P 네트워크

// block 클래스랑 chain을 가지고 P2P 네트워크 http / ws 환경 구성

// http / ws 환결 구성

// P2P 네트워크를 만드는데 http와 ws 웹소켓을 사용해서 API를 구성하고 블록을 가져올 express 모듈로 서버 동작을 하고
// P2P 네트워크를 웹소켓으로 구성
